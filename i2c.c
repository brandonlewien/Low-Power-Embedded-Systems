...
void I2C_Setup(void) {
	...
	I2C0->CTRL |= I2C_CTRL_AUTOACK;													// Auto ACK
...
uint8_t I2C_Read_from_Reg(uint8_t slave_addr_rw, uint8_t cmd){
	...
	I2C0->TXDATA = slave_addr_rw + I2C_WRITE;										// send slave addr in upper 7 bits and WRITE bit in LSB to send command before reading
	...
	I2C0->TXDATA = slave_addr_rw + I2C_READ;										// send slave addr and READ bit
	while(!(I2C0->IF & I2C_IF_ACK));												// wait for ACK from slave
	I2C0->IFC |= I2C_IFC_ACK;														// clear ACK flag

	while(!(I2C0->IF & I2C_IF_RXDATAV));											// wait for entire byte to be transmitted and available in RX buffer
	data = I2C0->RXDATA; 															// read data from RX buffer
